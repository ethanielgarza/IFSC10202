...
def find_distance():
    ...
    # Search for city indices
    from_city_index = -1
    to_city_index = -1

    # Find the index of the 'From City' in the zeroth column
    for i in range(len(data)):
        if data[i][0].strip().lower() == from_city.strip().lower():
            from_city_index = i
            break

    # Find the index of the 'To City' in the zeroth row
    if from_city_index != -1:
        for j in range(len(data[0])):
            if data[0][j].strip().lower() == to_city.strip().lower():
                to_city_index = j
                break

    # Display results based on whether cities were found
    if from_city_index == -1:
